...
async def catalog_handler(callback: CallbackQuery):
    ...
    for index, category in enumerate(categories):
        builder.button(
            text=category,
            callback_data=f"category_idx:{index}"
        )
...
@router.callback_query(F.data.startswith("category_idx:"))
async def category_index_handler(callback: CallbackQuery):
    try:
...
async def product_handler(callback: CallbackQuery):
    ...
    categories = get_categories()
    category_index = categories.index(product["category"]) if product["category"] in categories else -1
    back_callback = f"category_idx:{category_index}" if category_index >= 0 else "catalog"
